...
from utils import os1_utils, setup_utils
...
def update(instance):
    ...
    with settings(host_string=host_string, key_filename=args.key_file):
        # Backup the cloud.cfg file since the update might overwrite it
        run('sudo cp /etc/cloud/cloud.cfg ~/cloud.cfg', warn_only=True)
        ...
        # Restore cloud.cfg
        run('sudo cp ~/cloud.cfg /etc/cloud/cloud.cfg', warn_only=True)
